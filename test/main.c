...
int main()
{
	...
	errorCode = vn100_connect(
		...
		COM_PORT,
		BAUD_RATE);
	printf("vn100_connect error_code: %d\n", errorCode);
	...
	errorCode = vn100_setAsynchronousDataOutputType(
        ...
        VNASYNC_OFF,
        true);
	printf("vn100_setAsynchronousDataOutputType error_code: %d\n", errorCode);
	...
	errorCode = vn100_setBinaryOutput1Configuration(
		...
		1,							/* Outputting binary data at 4 Hz (800 Hz on-board filter / 200 = 4 Hz). */
		//BG1_NONE,
		...
		BG5_NONE,
		true);
	printf("vn100_setBinaryOutput1Configuration error_code: %d\n", errorCode);
	...
	printf("vn100_registerAsyncDataReceivedListener error_code: %d\n", errorCode);
